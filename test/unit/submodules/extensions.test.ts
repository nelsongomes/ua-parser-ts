...
import { Bots, CLIs, Crawlers, Emails, Fetchers, InApps, Libraries, Vehicles } from '../../../src/extensions/ua-parser-extensions';
import { BrowserType, OSName, Extension } from '../../../src/enums/ua-parser-enums';
        ...
        describe(desc as string, () => {
            tests.forEach((test: any) => {
                it(`Can detect ${test.desc}: "${test.ua}"`, () => {
                    const { browser, device } = UAParser(test.ua, ext as any);
                    if ('browser' in (ext as any)) {
...
describe('Testing the safety of regexes', () => {
    ...
    let regexes: string[];
    let code = fs.readFileSync('src/extensions/ua-parser-extensions.ts', 'utf8').toString();
    let ast = parseJS(code, { sourceType: 'module', plugins: ['typescript'] });
    regexes = [];
    traverse(ast, {
        RegExpLiteral: (path) => {
