...
import { CLIs } from '../../src/extensions/ua-parser-extensions';
import { Extension } from '../../src/enums/ua-parser-enums';

const browsers = require('../data/ua/browser/browser-all.json');
const cpus     = require('../data/ua/cpu/cpu-all.json');
const engines  = require('../data/ua/engine/engine-all.json');
...
const devices = readJsonFiles('test/data/ua/device');
const os      = readJsonFiles('test/data/ua/os');

describe('UAParser()', function () {
    it('should equal new UAParser().setUA(ua).getResult()', function () {
...
describe('UAParser get*() methods', () => {
    ...
    .forEach(method => {
        describe(`[${method.title}]`, () => {
            method.list.forEach((unit: any) => {
                describe(`[${unit.desc}]: "${unit.ua}"`, () => {
                    const actual = (UAParser(unit.ua) as any)[method.label];
                    Object.entries(unit.expect).forEach(entry => {
                        const [key, val] = entry;
                        it(`Should return ${key}: ${val}`, () => {
                            assert.strictEqual(String(val), String((actual as any)[key]));
                        });
                    });
                });
            });
...
describe('Returns', function () {
    ...
    it('works even when Array.prototype has been mangled', function(done) {
        ...
        function withMangledArrayProto(fn: () => any, key = 'isEmpty', value: any = function() { return (this as any).length === 0; }) {
            const originalValue = (Array.prototype as any)[key];
            const restore = Object.hasOwnProperty.call(Array.prototype, key)
                ? () => (Array.prototype as any)[key] = originalValue
                : () => delete (Array.prototype as any)[key];

            (Array.prototype as any)[key] = value;
            const result = fn();
            restore();
...
describe('useExtension()', () => {
    ...
    it('Prioritize last input', () => {
        ...
        uap
          .setUA('MyBrowzer')
          .useExtension(ext1 as any)
          .useExtension(ext2 as any);
...
describe('Extending Regex', function () {
    ...
    const myOwnBrowser = [[/(myownbrowser)\/((\d+)?[\w\.]+)/i], [UAParser.BROWSER.NAME, UAParser.BROWSER.VERSION, UAParser.BROWSER.MAJOR]];

    it('should parse custom browser regex with ua string', function () {
        const parser1 = new UAParser(uaString, {browser: myOwnBrowser} as any);
    ...
    it('should parse custom browser regex with setUA', function () {
        const parser2 = new UAParser({browser: myOwnBrowser} as any);
    ...
    it('should support custom browser type in extension', function () {
        const myOwnListOfBrowsers = [
            [/(mybrowser)\/([\w\.]+)/i], [UAParser.BROWSER.NAME, UAParser.BROWSER.VERSION, ['type', 'bot']]
        ];
        const myParser = new UAParser({ browser: myOwnListOfBrowsers } as any);
    ...
    it('should support custom device regex in extension', function () {
        const myOwnListOfBrowsers = [
            [/(mybrowser)\/([\w\.]+)/i], [UAParser.BROWSER.NAME, UAParser.BROWSER.VERSION, ['type', 'bot']]
        ];
    ...
    it('should support array of extensions', function () {
        const myOwnListOfBrowsers = [
            [/(mybrowser)\/([\w\.]+)/i], [UAParser.BROWSER.NAME, UAParser.BROWSER.VERSION, ['type', 'bot']]
        ];
        const myOwnListOfDevices = [
...
describe('Testing regexes', function () {
    ...
    beforeAll(function () {
        const code = fs.readFileSync('src/main/ua-parser.ts', 'utf8').toString();
        const ast = parseJS(code, { sourceType: "module", plugins: ['typescript'] });
        regexes = [];
        traverse(ast, {
            RegExpLiteral: (path) => {
    ...
    it('all regexes should pass safe-regex', function () {
        for (const regex of regexes) {
            assert.strictEqual(safe(regex), true, `Regex failed safe-regex check: ${regex}`);
        }
    });
});
...
describe('Read user-agent data from req.headers', function () {
    ...
    it('Can be called with UAParser(extensions, headers)', function () {
        let engine = UAParser(ext as any, req.headers).engine;
    ...
    if (typeof Headers !== 'undefined') {
        it('Fetch API\'s Header can be passed directly into headers', () => {
            ...
            const { browser } = UAParser(reqHeaders as any);
